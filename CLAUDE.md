...
| **認知進化引擎** | 覺察到新認知**後** | 先改程式碼 → 再寫 CLAUDE.md / 建 skill | `.claude/skills/cognitive-evolution.md` |

前者確保「每個情境都有適配的工作流程」，後者確保「每次學到的東西都沉澱到系統裡」。兩個引擎形成閉環：認知進化建出的新 skill，會被工作流程覺察器在下次工作時自動檢索到。
...
| 開始任何非 trivial 工作前 | `.claude/skills/workflow-awareness.md` | 識別情境 → 檢索對應 skill → 載入執行 |
| 覺察到新認知（跨 App 共通問題、production 數據揭示模式、用戶糾正做法） | `.claude/skills/cognitive-evolution.md` | 先改程式碼 → 再寫 CLAUDE.md / 建 skill |
