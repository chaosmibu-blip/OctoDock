...
AgentDock 是一個面向非技術用戶的基礎設施產品。用戶只需設定一個 MCP URL，就能讓任何 AI agent 操作所有已授權的 App，並且擁有跨 agent 共享的操作記憶。

## 產品定位

- **服務對象**：不懂程式、不懂英文，但重度依賴 AI agent 工作的人
- **核心價值**：一次串接全部能用 + 跨模型共享記憶 + 越用越懂你
- **競品差異**：Composio 服務開發者（需寫程式），AgentDock 服務終端用戶（只需貼 URL）
...
單一入口：`https://agentdock.app/mcp/{user_mcp_api_key}`

Agent 連上後，AgentDock 根據 api_key 辨識用戶，回傳該用戶已授權的所有工具。工具命名格式：`{app}_{action}`（如 `notion_create_page`）。
...
- AgentDock 系統: memory_query, memory_store, list_apps (3)
...
這是支撐 AgentDock 擴展到大量 App 的關鍵設計。每個 App 是一個獨立的 Adapter 模組，實作統一介面。核心系統自動掃描、自動註冊、自動載入。
...
- **Phase 4（Bot 自動回覆 + 擴展）**：Bot 自主回覆引擎（AgentDock 呼叫 LLM）+ 對話歷史 + Bot 人設 + LLM 費用機制 + 更多 App
...
- **Phase 4 的自動回覆**：需要新增 Webhook 接收端點、bot_configs 資料表、LLM 呼叫引擎。AgentDock 在這個場景下不再只是工具橋梁，它自己變成了一個 agent。
- **LLM 費用**：Phase 1-3 不需要 AgentDock 付 LLM 費用（agent 是用戶自己的）。Phase 4 自動回覆需要 AgentDock 自己呼叫 LLM，費用處理：用戶自帶 API key 或 AgentDock 代付轉嫁到訂閱費。

## 市場定位（2026.03 更新）

- GPT-5.4 的原生電腦操控能力 **不威脅** AgentDock：API 呼叫成功率 99%、毫秒級；電腦操控成功率 75%、秒級。有 API 的 App 永遠該走 API。
- GPT-5.4 的工具搜尋機制 **驗證** AgentDock 的方向：agent 面對太多工具會出問題，需要智慧篩選。
- 各 App 官方 MCP Server 只包裝了 API 功能的 20-40%。AgentDock 自己寫 Adapter，可以根據用戶實際需求覆蓋更多功能。這是自建 Adapter 的核心價值。
- 模型越強 → 越多人用 agent → AgentDock 的需求越大。
...
AgentDock 的解法：不是把所有工具都塞給 agent，而是根據記憶動態篩選。
...
Agent 連上 AgentDock，用戶有 60 個工具
  → AgentDock 根據：
    ...
    3. 當前脈絡（最近在做什麼、現在星期幾）
  → 只回傳最相關的 8-12 個工具
  → 其餘工具以一句話帶過，agent 需要時呼叫 agentdock_discover_tools 展開
```
...
詳細的資料庫 schema、OAuth 流程、工具定義 → 讀 `agentdock-spec.md`（專案根目錄）
