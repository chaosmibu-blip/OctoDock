...
| 檢查項 | 怎麼查 | 常見問題 |
|--------|--------|----------|
| **shadcn/ui 元件** | 新增的按鈕/輸入框/卡片/Modal 是否用 shadcn/ui 元件 | 手寫 className 而不用 `<Button>` / `<Input>` / `<Card>` |
| 色彩系統 | grep 所有 hex/rgb，確認只用設計系統定義的顏色（見 `visual-design.md`） | 隨手寫的灰色 `#999` 和 `#9CA3AF` 混用 |
...
| **美學清單** | 過渡動畫、視覺層次、互動回饋、空狀態、細節打磨（詳見 `visual-design.md`） | 沒有 hover 效果、loading 是空白、空狀態沒處理 |
